...
print 'Indice por Reconstruir ' + @name +' de la tabla '+  @object_name
--ejecucion de la reindexacion
SELECT @gozalo = 'ALTER INDEX ['+ RTRIM (@name) + '] ON [' + @esquema + '].[' +  RTRIM (@object_name) + '] REBUILD PARTITION = ALL WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, FILLFACTOR = 80)'
   exec (@gozalo)

--captura de errorres script de Microsoft
